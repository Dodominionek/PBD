...
for ch in data_sequence[sequence_size:len(data_sequence):1]:
    ...
    if i != 0:
        ...
        hits.append(hit)
        positions.append(i)
        sizes.append(sys.getsizeof(sequence_occurences) + sys.getsizeof(sequence_predictions) + sys.getsizeof(sequence_probability))
        data_to_plot[i] = hit
...
for elem in sizes:
    ...
    pdd['size [bytes]'].append(elem)
    # print(pdd)
    pdd['characters'].append(j) 
    j += 1

pd_data = pd.DataFrame(pdd, columns=['characters', 'size [bytes]'])

# pd_data.drop(pd_data.loc[pd_data['characters'] < sequence_size].index, inplace=True)

print(pd_data)
g = ggplot(pd_data) + aes(x='characters', y='size [bytes]') + geom_line()
print(g)
